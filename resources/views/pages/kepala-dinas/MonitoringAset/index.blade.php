<x-app-layout>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sm:p-6 mb-6">
        ...
        @elseif($mode === 'nonaktif')
            <form action="{{ route($routeName) }}" method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-3">
                <div>
                    <label for="jenis" class="block text-[10px] font-bold text-slate-400 tracking-wider uppercase mb-2">Jenis Aset</label>
                    <select id="jenis" name="jenis" onchange="this.form.submit()" class="w-full bg-white border border-slate-200 text-slate-700 text-xs rounded-xl px-4 py-3 appearance-none focus:outline-none focus:border-[#0F3092] transition-colors font-medium">
                ...
                <div class="md:col-span-3">
                    <label for="search" class="block text-[10px] font-bold text-slate-400 tracking-wider uppercase mb-2">Cari Aset</label>
                    <div class="relative">
                        <input type="text" id="search" name="search" value="{{ $filters['search'] }}" placeholder="Cari nama aset, kode aset, kategori, bidang, atau kondisi..." class="w-full bg-white border border-slate-200 text-slate-700 text-xs rounded-xl pl-10 pr-10 py-3 focus:outline-none focus:border-[#0F3092] transition-colors font-medium">
                        <div class="absolute inset-y-0 left-0 flex items-center pl-3.5 text-slate-400 pointer-events-none">
                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                            </svg>
        ...
        <div class="flex flex-col sm:flex-row justify-between gap-3">
            <div>
                <h3 class="text-base font-bold text-slate-800 tracking-tight">
                    {{ $mode === 'nonaktif' ? 'Daftar Aset Nonaktif' : 'Daftar Aset Terpantau' }}
                </h3>
                <p class="text-xs text-slate-400 mt-1">
                    {{ $mode === 'nonaktif' ? 'Aset yang keluar dari inventaris aktif tetap tersimpan sebagai audit trail.' : 'Seluruh data diambil dari aset terverifikasi dan masih aktif.' }}
                </p>
            </div>
            <span class="inline-flex items-center self-start rounded-full bg-slate-50 border border-slate-200 px-3 py-1.5 text-[11px] font-bold text-slate-600">
                {{ $assets->total() }} {{ $mode === 'nonaktif' ? 'Riwayat' : 'Data' }}
            </span>
        </div>
        ...
        <div class="responsive-table">
            ...
            @elseif($mode === 'nonaktif')
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-slate-200 text-[10px] font-extrabold text-slate-400 uppercase tracking-wider">
                            <th class="py-4 px-4 w-[30%]">Aset</th>
                            <th class="py-4 px-4">Jenis</th>
                            <th class="py-4 px-4">Bidang</th>
                            <th class="py-4 px-4">Kondisi</th>
                            <th class="py-4 px-4">Nilai Buku</th>
                            <th class="py-4 px-4 w-[24%]">Informasi Nonaktif</th>
                    ...
                    <tbody class="divide-y divide-slate-100 text-xs text-slate-700">
                        @forelse($assets as $asset)
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="py-4 px-4">
                                    <span class="text-xs font-bold text-slate-700 uppercase tracking-wide">{{ $asset->type_label }}</span>
                                </td>
                                <td class="py-4 px-4 font-semibold text-slate-600">{{ $asset->bidang->nama_bidang ?? '-' }}</td>
                                <td class="py-4 px-4">
                                    <span class="text-xs font-bold text-slate-700 uppercase tracking-wide">{{ $asset->condition ?? '-' }}</span>
                                </td>
                                <td class="py-4 px-4 font-bold text-slate-800">{{ $formatCurrency($asset->book_value) }}</td>
                                <td class="py-4 px-4">
                                    <div class="font-bold text-slate-800">{{ $asset->deleted_at ? $asset->deleted_at->format('d M Y') : '-' }}</div>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="py-8 px-4 text-center text-slate-400 font-medium bg-slate-50/50">
                                    Belum ada data aset nonaktif yang cocok dengan filter monitoring.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
