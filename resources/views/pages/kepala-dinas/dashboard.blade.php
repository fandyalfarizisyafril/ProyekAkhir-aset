<x-app-layout>
    @php
        ...
        $bookValuePercent = $summary['totalRegisterValue'] > 0 ? round(($summary['bookValue'] / $summary['totalRegisterValue']) * 100, 1) : 0;
    @endphp
    ...
    <div class="mb-6">
        ...
        <p class="text-sm text-slate-500 mt-1">
            Visualisasi strategis aset Diskominfotik Provinsi Riau berdasarkan data terverifikasi.
        </p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        />
        <x-dashboard.stats-card
            title="Nilai Aset Register"
            value="{{ $formatCurrency($summary['totalRegisterValue']) }}"
            trend="{{ $formatNumber($summary['calculatedDepreciationCount']) }} aset tersusutkan"
            type="success"
        />
        <x-dashboard.stats-card
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 sm:p-6">
                <form action="{{ route('kepala-dinas.dashboard') }}" method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-3 md:items-end">
                    <div>
                ...
                @if($filters['bidang_id'] !== 'Semua Bidang' || $filters['kategori'] !== 'Semua Kategori' || $filters['kondisi'] !== 'Semua Kondisi' || (int) $filters['tahun'] !== now()->year)
                    <a href="{{ route('kepala-dinas.dashboard') }}" class="inline-block mt-4 text-[#0F3092] hover:text-[#0B2F83] text-xs font-semibold hover:underline">
                        Reset Filter
                    </a>
                @endif
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 sm:p-6">
                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Beban Penyusutan {{ $filters['tahun'] }}</span>
                    <div class="text-xl sm:text-2xl font-extrabold text-slate-800 mt-2">{{ $formatCurrency($summary['depreciationExpense']) }}</div>
                    <p class="text-xs text-slate-400 font-medium mt-1">Berdasarkan snapshot tahun terpilih</p>
                </div>
                <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 sm:p-6">
                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Nilai Buku Register</span>
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3">
                    <div>
                        <h3 class="text-base font-bold text-slate-800 tracking-tight">
                            Statistik Aset Per Bidang
                        </h3>
                        <p class="text-xs text-slate-400 mt-0.5">
                            Distribusi unit dan nilai aset Register berdasarkan bidang.
                        </p>
                    </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 sm:p-6">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-5 gap-3">
                    <div>
                        <h3 class="text-base font-bold text-slate-800 tracking-tight">
                            Aset Register Bernilai Tertinggi
                        </h3>
                        <p class="text-xs text-slate-400 mt-0.5">
                            Prioritas pemantauan aset berdasarkan nilai perolehan.
                        </p>
                    </div>
                </div>
                ...
                <div class="responsive-table">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-100 text-xs text-slate-700">
                            @forelse($topRegisterAssets as $asset)
                                <tr class="hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td class="py-3 px-3 font-semibold text-slate-600">{{ $asset->bidang->nama_bidang ?? '-' }}</td>
                                    <td class="py-3 px-3 font-semibold text-slate-600">{{ $asset->condition }}</td>
                                    <td class="py-3 px-3 font-bold text-slate-800">{{ $formatCurrency($asset->value) }}</td>
                                    <td class="py-3 px-3 font-bold text-[#0F3092]">{{ $formatCurrency($asset->book_value) }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="py-8 px-3 text-center text-slate-400 font-medium bg-slate-50/50">
                                        Belum ada aset Register terverifikasi untuk filter ini.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 sm:p-6">
                ...
                <div class="space-y-3 text-xs text-slate-600">
                    <div class="flex justify-between border-b border-slate-100 pb-2">
                        ...
                        <strong class="text-emerald-700">{{ $goodPercent }}%</strong>
                    </div>
                    <div class="flex justify-between border-b border-slate-100 pb-2">
                        <span>Register Tersusutkan</span>
                        <strong class="text-slate-800">{{ $formatNumber($summary['calculatedDepreciationCount']) }}</strong>
                    </div>
                    <div class="flex justify-between">
                        <span>Nilai Buku Register</span>
                        <strong class="text-slate-800">{{ $bookValuePercent }}%</strong>
                    </div>
                </div>
            </div>
        </div>
