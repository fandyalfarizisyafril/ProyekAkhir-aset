<x-app-layout>
    ...
    <div class="mb-6">
        <h2 class="text-2xl font-extrabold text-slate-800 tracking-tight">
            Pendaftaran Aset Baru Register / Fisik
        </h2>
    ...
    <!-- Main Stack Form -->
    <form action="{{ route('admin-perbidang.data-aset-register.store') }}" method="POST" id="create-asset-form" class="space-y-8 max-w-7xl">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        >
                            <option value="Baik" {{ old('status_barang', 'Baik') === 'Baik' ? 'selected' : '' }}>Baik</option>
                            <option value="Rusak Ringan" {{ old('status_barang') === 'Rusak Ringan' ? 'selected' : '' }}>Rusak Ringan</option>
                            <option value="Rusak Berat" {{ old('status_barang') === 'Rusak Berat' ? 'selected' : '' }}>Rusak Berat</option>
                        </select>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        >
                            <option value="Dihapus / Bakar" {{ old('metode_pemusnahan') === 'Dihapus / Bakar' ? 'selected' : '' }}>Dihapus / Bakar</option>
                            <option value="Penjualan" {{ old('metode_pemusnahan') === 'Penjualan' ? 'selected' : '' }}>Penjualan</option>
                            <option value="Hibah" {{ old('metode_pemusnahan') === 'Hibah' ? 'selected' : '' }}>Hibah</option>
                            <option value="Lainnya" {{ old('metode_pemusnahan') === 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                        </select>
                        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-slate-400">
                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
                ...
                <div>
                    ...
                    <div class="flex flex-wrap items-center gap-x-8 gap-y-2 mt-2">
                        <label class="flex items-center cursor-pointer text-xs font-bold text-slate-700 select-none">
                            <input type="radio" name="kerahasiaan" value="Umum" {{ old('kerahasiaan', 'Umum') === 'Umum' ? 'checked' : '' }} class="h-4 w-4 text-[#002D84] border-slate-300 focus:ring-[#002D84] mr-2">
                            <span>Umum</span>
                        </label>
                        <label class="flex items-center cursor-pointer text-xs font-bold text-slate-700 select-none">
                            <input type="radio" name="kerahasiaan" value="Terbatas" {{ old('kerahasiaan') === 'Terbatas' ? 'checked' : '' }} class="h-4 w-4 text-[#002D84] border-slate-300 focus:ring-[#002D84] mr-2">
                            <span>Terbatas</span>
                        </label>
                        <label class="flex items-center cursor-pointer text-xs font-bold text-slate-700 select-none">
                            <input type="radio" name="kerahasiaan" value="Rahasia" {{ old('kerahasiaan') === 'Rahasia' ? 'checked' : '' }} class="h-4 w-4 text-[#002D84] border-slate-300 focus:ring-[#002D84] mr-2">
                            <span>Rahasia</span>
                        </label>
                    </div>
                    @error('kerahasiaan')
                ...
                <div>
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        ...
                        <label class="relative cursor-pointer">
                            <input type="radio" name="kritikalitas" value="RENDAH" {{ old('kritikalitas', 'RENDAH') === 'RENDAH' ? 'checked' : '' }} class="sr-only peer">
                        ...
                        <!-- Sedang -->
                        <label class="relative cursor-pointer">
                            <input type="radio" name="kritikalitas" value="SEDANG" {{ old('kritikalitas') === 'SEDANG' ? 'checked' : '' }} class="sr-only peer">
                        ...
                        <!-- Tinggi -->
                        <label class="relative cursor-pointer">
                            <input type="radio" name="kritikalitas" value="TINGGI" {{ old('kritikalitas') === 'TINGGI' ? 'checked' : '' }} class="sr-only peer">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-6">
                ...
                <!-- Tanggal Perolehan -->
                <div>
                    <label for="tanggal_perolehan" class="block text-[10px] font-bold text-slate-400 tracking-wider uppercase mb-2">
                        TANGGAL PEROLEHAN
                    </label>
                    <input
                        type="date"
                        ...
                        value="{{ old('tanggal_perolehan', now()->toDateString()) }}"
                        max="{{ now()->toDateString() }}"
                        class="w-full bg-slate-50 border @error('tanggal_perolehan') border-red-300 focus:border-red-500 @else border-slate-200 focus:border-[#0F3092] @enderror text-slate-700 text-xs rounded-xl px-4 py-3.5 focus:outline-none transition-colors font-medium"
                    >
            ...
            if (form) {
                form.addEventListener('submit', function (e) {
                    ...
                    Swal.fire({
                        ...
                        text: 'Apakah Anda yakin data aset register yang dimasukkan sudah benar?',
                        icon: 'question',
