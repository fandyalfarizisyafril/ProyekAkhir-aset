<x-app-layout>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        ...
        <div class="flex items-center space-x-3 w-full sm:w-auto">
            ...
            <a href="{{ route('admin-perbidang.data-aset-smki.create') }}" class="w-1/2 sm:w-auto bg-[#002D84] hover:bg-[#0B2F83] text-white text-xs font-bold uppercase tracking-wider px-5 py-3 rounded-xl flex items-center justify-center space-x-2 transition-all duration-150 shadow-sm">
                <!-- Plus Icon -->
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mb-8 space-y-6">
        <!-- Tidy Filter Form -->
        <form action="{{ route('admin-perbidang.data-aset-smki.index') }}" method="GET" id="filter-form" class="space-y-4">
            <!-- Dropdowns & Reset row -->
            <div class="flex flex-col sm:flex-row sm:items-center gap-3">
                ...
                @if($search || ($kategori && $kategori !== 'Semua Kategori') || ($status && $status !== 'Semua Status'))
                    <a href="{{ route('admin-perbidang.data-aset-smki.index') }}" class="text-[#0F3092] hover:text-[#0B2F83] text-xs font-semibold whitespace-nowrap pl-2 hover:underline">
                        Reset Filter
                    </a>
                @endif
            </div>
                ...
                <tbody class="divide-y divide-slate-100 text-xs text-slate-700">
                    @forelse($assets as $asset)
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <!-- Nama Aset (Merk/Model + subtext) -->
                            <td class="py-4 px-4">
                                <div class="font-bold text-slate-800 text-sm">
                                    {{ $asset->merk_model }}
                                </div>
                                <div class="text-[10px] text-slate-400 mt-1 flex flex-wrap gap-x-2">
                                    <span>S/N: <strong class="text-slate-600 font-semibold">{{ $asset->no_ser_model ?? '-' }}</strong></span>
                                    <span>|</span>
                                    <span>Ruangan: <strong class="text-slate-600 font-semibold">{{ $asset->ruangan ?? '-' }}</strong></span>
                                    <span>|</span>
                                    <span>PJ: <strong class="text-slate-600 font-semibold">{{ $asset->penanggung_jawab }}</strong></span>
                                </div>
                            </td>
                            ...
                            <td class="py-4 px-4 text-center">
                                <div class="flex items-center justify-center space-x-3">
                                    <!-- View/Detail Icon (Mapped to edit page for simplicity in mockup) -->
                                    <a href="{{ route('admin-perbidang.data-aset-smki.edit', $asset->id) }}" class="text-slate-400 hover:text-slate-600 transition-colors p-1 hover:bg-slate-100 rounded" title="Detail Aset">
                                    ...
                                    <!-- Edit Link -->
                                    <a href="{{ route('admin-perbidang.data-aset-smki.edit', $asset->id) }}" class="text-[#0F3092] hover:text-blue-800 transition-colors p-1 hover:bg-blue-50 rounded" title="Edit Aset">
                                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    ...
                                    <!-- Delete Link -->
                                    <form action="{{ route('admin-perbidang.data-aset-smki.destroy', $asset->id) }}" method="POST" class="inline delete-form" data-asset-name="{{ $asset->merk_model }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-600 hover:text-red-700 transition-colors p-1 hover:bg-red-50 rounded cursor-pointer" title="Hapus Aset">
                                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            deleteForms.forEach(form => {
                form.addEventListener('submit', function (e) {
                    ...
                    Swal.fire({
                        ...
                        text: `Aset "${assetName}" akan dihapus secara permanen dari database!`,
                        icon: 'warning',
