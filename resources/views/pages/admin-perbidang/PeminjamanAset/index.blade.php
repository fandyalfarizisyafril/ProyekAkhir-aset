<x-app-layout>
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-1">
                Ajukan peminjaman aset terverifikasi untuk diverifikasi Super Admin.
            </p>
        </div>

        <a href="{{ route('admin-perbidang.peminjaman-aset.create') }}" class="w-full sm:w-auto bg-[#002D84] hover:bg-[#0B2F83] text-white text-xs font-bold uppercase tracking-wider px-5 py-3 rounded-xl flex items-center justify-center space-x-2 transition-all duration-150 shadow-sm">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mb-8 space-y-6">
        <form action="{{ route('admin-perbidang.peminjaman-aset.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div class="relative md:col-span-2">
                <input
                    ...
                    name="search"
                    value="{{ $search }}"
                    placeholder="Cari nama aset, kode aset, atau keperluan..."
        ...
        @if($search || $status !== 'Semua Status')
            <a href="{{ route('admin-perbidang.peminjaman-aset.index') }}" class="inline-block text-[#0F3092] hover:text-[#0B2F83] text-xs font-semibold hover:underline">
                Reset Filter
            </a>
        @endif
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="border-b border-slate-200 text-[10px] font-extrabold text-slate-400 uppercase tracking-wider">
                        ...
                        <th class="py-4 px-4">Bidang Aset</th>
                        <th class="py-4 px-4">Tanggal Pinjam</th>
                    ...
                    @forelse($peminjaman as $item)
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="py-4 px-4 text-center">
                                <a href="{{ route('admin-perbidang.peminjaman-aset.show', $item->id) }}" class="inline-flex text-slate-500 hover:text-slate-700 transition-colors p-1 hover:bg-slate-100 rounded" title="Detail Peminjaman">
