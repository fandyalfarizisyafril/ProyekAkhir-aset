<x-app-layout>
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-1">
                Tinjau kebutuhan aset dari bidang peminta dan pilih aset yang dapat dimutasikan.
            </p>
        </div>

        <a href="{{ route('super-admin.verifikasi-mutasi.index') }}" class="w-full sm:w-auto border border-slate-200 hover:bg-slate-50 text-slate-600 text-xs font-bold uppercase tracking-wider px-5 py-3 rounded-xl flex items-center justify-center transition-all duration-150 shadow-sm">
            Mutasi Keluar
        </a>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <x-dashboard.stats-card title="Menunggu" value="{{ number_format($pendingCount) }}" trend="Butuh pemilihan aset" type="info" />
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mb-8 space-y-6">
        <form action="{{ route('super-admin.permintaan-mutasi.index') }}" method="GET" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
                ...
                <div class="relative">
                    <input type="text" name="search" value="{{ $filters['search'] }}" placeholder="Cari kebutuhan, kategori, pemohon..." class="w-full bg-white border border-slate-200 text-slate-700 text-xs rounded-xl pl-10 pr-10 py-3 focus:outline-none focus:border-[#0F3092] transition-colors font-medium">
            ...
            @if($filters['search'] || $filters['jenis'] !== 'Semua Jenis' || $filters['status'] !== 'Menunggu Verifikasi' || $filters['bidang_id'] !== 'Semua Bidang')
                <a href="{{ route('super-admin.permintaan-mutasi.index') }}" class="inline-block text-[#0F3092] hover:text-[#0B2F83] text-xs font-semibold hover:underline">Reset Filter</a>
            @endif
        </form>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="border-b border-slate-200 text-[10px] font-extrabold text-slate-400 uppercase tracking-wider">
                        ...
                        <th class="py-4 px-4">Bidang Peminta</th>
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            <td class="py-4 px-4">
                                <div class="flex items-center gap-2 mb-1">
                                    <span class="px-2 py-0.5 text-[9px] font-extrabold tracking-wider rounded-md bg-[#EBF3FF] text-[#0F3092] border border-[#CBD5E1]">{{ strtoupper($item->jenis_aset) }}</span>
                                    <span class="font-bold text-slate-800 text-sm">{{ $item->nama_kebutuhan }}</span>
                                </div>
                                <div class="text-[10px] text-slate-400 font-semibold">{{ $item->kategori_aset }}</div>
                            </td>
                            <td class="py-4 px-4 font-semibold text-slate-600">{{ $item->bidangPeminta->nama_bidang ?? '-' }}</td>
                            <td class="py-4 px-4 font-semibold text-slate-600">{{ $item->tanggal_permintaan?->format('d M Y') }}</td>
                            <td class="py-4 px-4">
                                <div class="font-bold text-slate-700">{{ $item->peminta->nama ?? '-' }}</div>
                                <div class="text-[10px] text-slate-400 mt-1">{{ optional($item->created_at)->format('d M Y H:i') }}</div>
                            </td>
                            <td class="py-4 px-4">
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold leading-5 {{ $statusClass }}">{{ $item->status }}</span>
                            </td>
                            <td class="py-4 px-4 text-center">
                                <a href="{{ route('super-admin.permintaan-mutasi.show', $item->id) }}" class="inline-flex text-slate-500 hover:text-slate-700 transition-colors p-1 hover:bg-slate-100 rounded" title="Tinjau Permintaan">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="py-8 px-4 text-center text-slate-400 font-medium bg-slate-50/50">
                                Tidak ada permintaan mutasi yang cocok dengan filter.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if($permintaan->hasPages())
            <div class="border-t border-slate-100 pt-4 flex flex-col sm:flex-row justify-between items-center text-xs font-semibold text-slate-500 gap-4">
                <div>Menampilkan {{ $permintaan->firstItem() ?? 0 }}-{{ $permintaan->lastItem() ?? 0 }} dari {{ $permintaan->total() }} permintaan</div>
