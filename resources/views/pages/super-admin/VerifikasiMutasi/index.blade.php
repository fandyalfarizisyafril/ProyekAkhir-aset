    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mb-8 space-y-6">
        <form action="{{ route('super-admin.verifikasi-mutasi.index') }}" method="GET" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
                ...
                <div class="relative">
                    <input
                        ...
                        placeholder="Cari aset, kode, pemohon..."
            ...
            @if($filters['search'] || $filters['jenis'] !== 'Semua Jenis' || $filters['status'] !== 'Menunggu Verifikasi' || $filters['bidang_id'] !== 'Semua Bidang')
                <a href="{{ route('super-admin.verifikasi-mutasi.index') }}" class="inline-block text-[#0F3092] hover:text-[#0B2F83] text-xs font-semibold hover:underline">
                    Reset Filter
                </a>
            @endif
        </form>
                ...
                <tbody class="divide-y divide-slate-100 text-xs text-slate-700">
                    @forelse($mutasi as $item)
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="py-4 px-4">
                                <div class="font-semibold text-slate-700">{{ $item->bidangAsal->nama_bidang ?? '-' }}</div>
                                <div class="text-[10px] text-slate-400 my-1">ke</div>
                                <div class="font-semibold text-[#0F3092]">{{ $item->bidangTujuan->nama_bidang ?? '-' }}</div>
                            </td>
                            ...
                            <td class="py-4 px-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('super-admin.verifikasi-mutasi.show', $item->id) }}" class="text-slate-500 hover:text-slate-700 transition-colors p-1 hover:bg-slate-100 rounded" title="Detail Mutasi">
                                    ...
                                    @if($item->status === 'Menunggu Verifikasi')
                                        <form action="{{ route('super-admin.verifikasi-mutasi.approve', $item->id) }}" method="POST" class="inline verification-form" data-action="menyetujui" data-asset-name="{{ $assetName }}">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="text-emerald-600 hover:text-emerald-700 transition-colors p-1 hover:bg-emerald-50 rounded" title="Setujui Mutasi">
                                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                                                </svg>
                                            </button>
                                        </form>
                                        <form action="{{ route('super-admin.verifikasi-mutasi.reject', $item->id) }}" method="POST" class="inline verification-form" data-action="menolak" data-asset-name="{{ $assetName }}">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="text-red-600 hover:text-red-700 transition-colors p-1 hover:bg-red-50 rounded" title="Tolak Mutasi">
                                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
