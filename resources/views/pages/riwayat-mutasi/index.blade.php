    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-1">
                Histori perpindahan aset lengkap dengan tanggal, bidang asal, bidang tujuan, dan status verifikasi.
            </p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mb-8 space-y-6">
        <form action="{{ route('riwayat-mutasi.index') }}" method="GET" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-6 gap-3">
                ...
                <input
                    ...
                    class="w-full bg-white border border-slate-200 text-slate-600 text-xs rounded-xl px-4 py-3 focus:outline-none focus:border-[#0F3092] transition-colors font-medium"
                >
                ...
                <div class="relative">
                    <input
                        ...
                        placeholder="Cari aset, bidang, pemohon..."
            ...
            @if($filters['search'] || $filters['jenis'] !== 'Semua Jenis' || $filters['status'] !== 'Semua Status' || $filters['bidang_id'] !== 'Semua Bidang' || $filters['tanggal_mulai'] || $filters['tanggal_selesai'])
                <a href="{{ route('riwayat-mutasi.index') }}" class="inline-block text-[#0F3092] hover:text-[#0B2F83] text-xs font-semibold hover:underline">
                    Reset Filter
                </a>
            @endif
        </form>
                ...
                <tbody class="divide-y divide-slate-100 text-xs text-slate-700">
                    @forelse($mutasi as $item)
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="py-4 px-4 font-semibold text-[#0F3092]">
                            ...
                            <td class="py-4 px-4">
                                ...
                                <div class="text-[10px] text-slate-400 mt-1">{{ $item->pemohon->bidang->nama_bidang ?? '-' }}</div>
                            </td>
                            <td class="py-4 px-4">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="py-8 px-4 text-center text-slate-400 font-medium bg-slate-50/50">
                                Belum ada riwayat mutasi aset yang cocok dengan filter.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if($mutasi->hasPages())
            <div class="border-t border-slate-100 pt-4 flex flex-col sm:flex-row justify-between items-center text-xs font-semibold text-slate-500 gap-4">
                <div>
                    Menampilkan {{ $mutasi->firstItem() ?? 0 }}-{{ $mutasi->lastItem() ?? 0 }} dari {{ $mutasi->total() }} riwayat
                </div>
                <div>
                    {{ $mutasi->links() }}
                </div>
